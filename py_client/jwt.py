...
from typing import Optional #To handle red underline error
...
class JWTClient:
    ...
    def __post_init__(self):
        if self.cred_path.exists(): 
            ...
            if data is None:
                ...
                # Guard: if no access token, treat as invalid
                if not self.access:
                    self.clear_tokens()
                    self.perform_auth()
                    return
        ...
        if self.cred_path is not None:
            ...
            if self.access:
                try:
                    ...
                    # ignore write errors (e.g., path issues) to avoid crashing
                    pass
            else:
    ...
    def perform_refresh(self):
        ...
        r = requests.post(endpoint, json=data)  # no auth header needed for refresh endpoint
        if r.status_code != 200:
            self.clear_tokens()
            return False
        refresh_data = r.json()
        ...
        stored_data = {
            ...
            # prefer returned refresh if present, otherwise keep the existing one
            'refresh': refresh_data.get('refresh', self.refresh)
        }
